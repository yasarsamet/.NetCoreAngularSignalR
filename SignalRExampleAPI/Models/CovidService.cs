...
namespace SignalRExampleAPI.Models
{
    public class CovidService
    {
        ...
        public List<CovidChart> GetCovidChartList()
        {
            ...
            {
                command.CommandText= "select tarihh,[1],[2],[3],[4],[5] from " +
                    "(select[City],[Count], Cast([CovidDate] as date) as tarihh from " +
                    "Covids) as covidTable  Pivot (SUM(Count) for City IN ([1],[2],[3],[4],[5])) as ptable " +
                    "order by tarihh asc";
